...
pub fn run_headless(program_bytes: &[u8], program_start: u16) -> Result<()> {
    let mut bus = Bus::new();
    bus.write_word(cpu::RESET_VECTOR, program_start); // TODO: Include in the program
    bus.load(0x0000, program_bytes);
    let mut cpu = Cpu::new();
    cpu.reset();
    cpu.run(&mut bus, RunOption::StopOnBreakInstruction);
